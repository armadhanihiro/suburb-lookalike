...
from features_test import load_suburb_data_test as load_suburb_data  # 你的特征加载函数
...
def load_data():
    ...
    df, X, scaler, df_insufficient = load_suburb_data(client)
    return df, X, scaler, df_insufficient

df, X, scaler, df_insufficient = load_data()
...
if st.button("🔍 查找相似郊区"):
   ...
   #     name = df.loc[i, "sa2_name"]
    ...
    # st.subheader("🏘️ 最相似的 3 個郊區（最高分）")

    # 只取前 3 个
    top_3_idx = top_idx[:3]
    top_3_scores = scores[:3]
    for rank, (i, score) in enumerate(zip(top_3_idx, top_3_scores), start=1):
        name = df.loc[i, "display_name"]
        # 加星星、加粗、放大字号
        st.markdown(f"### ⭐ {rank}. **{name}** — 相似度: **{score:.1f} / 100**")

    # 如果你还想显示剩余的 7 个作为补充
    with st.expander("查看其餘 7 個相似郊區"):
        for rank, (i, score) in enumerate(zip(top_idx[3:], scores[3:]), start=4):
            name = df.loc[i, "display_name"]
            st.write(f"{rank}. {name} — 相似度: {score:.1f} / 100")