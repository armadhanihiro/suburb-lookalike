...
from features_test import load_suburb_data_test as load_suburb_data
...
if st.button("🔍 Find Similar Suburbs"):
    ...
    # Top 3 with stars and bold
    top_3_idx = top_idx[:3]
    top_3_scores = scores[:3]
    for rank, (i, score) in enumerate(zip(top_3_idx, top_3_scores), start=1):
        name = df.loc[i, "display_name"]
        # Add stars, bold, and larger font
        st.markdown(f"### ⭐ {rank}. **{name}** — Similarity: **{score:.1f} / 100**")

    # Remaining 7 in expander
    with st.expander("Check the other 7 similar suburbs"):
